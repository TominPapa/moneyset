// SettingsPage — RESET Budget V2
// 4탭: 일반 / 카테고리 / 자산·부채 / 데이터
...
function emptyAccount(): Account {
  ...
  return {
    ...
    balance: 0, isActive: true, sortOrder: 0,
...
function AssetsTab({ config, accounts, liabilities, onAccountsChange, onLiabilitiesChange }: AssetsTabProps) {
  ...
  async function deleteAccount() {
    ...
    setAccSaving(true);
    try { await onAccountsChange(accounts.filter((a) => a.id !== editAcc.id)); setAccSheet(false); }
    finally { setAccSaving(false); }
  }
  ...
  async function deleteLiability() {
    ...
    setLiabSaving(true);
    try { await onLiabilitiesChange(liabilities.filter((l) => l.id !== editLiab.id)); setLiabSheet(false); }
    finally { setLiabSaving(false); }
  }
    ...
    <div className={styles.tabContent}>
      ...
      <div className={styles.summaryRow}>
        <div className={styles.summaryCard}>
          ...
          <span className={styles.summaryValue} style={{ color: 'var(--income)' }}>{fmt(totalAssets)}</span>
        </div>
        <div className={styles.summaryCard}>
          <span className={styles.summaryLabel}>총 부채</span>
          <span className={styles.summaryValue} style={{ color: 'var(--expense)' }}>{fmt(totalLiab)}</span>
        </div>
        <div className={styles.summaryCard}>
          <span className={styles.summaryLabel}>순자산</span>
          <span
            className={styles.summaryValue}
            style={{ color: totalAssets - totalLiab >= 0 ? 'var(--text-primary)' : 'var(--expense)' }}
          >
          ...
          {liabilities.map((l) => (
            ...
            >
              ...
              {l.totalBalance !== undefined && (
                <span className={styles.listItemAmount} style={{ color: 'var(--expense)' }}>{fmt(l.totalBalance)}</span>
              )}
              <span className={styles.listItemArrow}>›</span>
            </div>
          ))}
...
function DataTab({ config }: DataTabProps) {
  ...
  async function handleInsertSeed() {
    ...
    try {
      ...
      setSeedMsg('✓ 시드 데이터 삽입 완료! 페이지를 새로고침하면 데이터가 보입니다.');
    } catch (e) {
  ...
  async function handleClearSeed() {
    ...
    try {
      await clearSeedData();
      setSeedMsg('✓ 전체 데이터 삭제 완료. 페이지를 새로고침하세요.');
    } catch (e) {
      setSeedMsg('오류: ' + String(e));
    } finally {
      setSeeding(false);
    ...
    <div className={styles.tabContent}>
      ...
      <div className={styles.section}>
        ...
        <p className={styles.sectionDesc}>
          Microsoft Excel에서 바로 열 수 있는 CSV 형식으로 저장합니다.
          UTF-8 BOM이 포함되어 한글이 정상 출력됩니다.
        </p>
      ...
      <div className={styles.section}>
        ...
        <p className={styles.sectionDesc}>
          카테고리, 결제수단, 고정지출 등 앱 설정 전체를 JSON 파일로 내보냅니다.
          백업이나 타 기기 복원에 활용할 수 있습니다.
        </p>
        <button
          className={styles.exportBtn}
      ...
      {/* 저장소 안내 */}
      <div className={styles.section}>
        <span className={styles.sectionTitle}>데이터 저장소 안내</span>
        <div className={styles.syncCard}>
          <div className={styles.syncRow}>
            <span className={styles.syncLabel}>거래 데이터</span>
            <span className={styles.syncValue}>IndexedDB (브라우저) + Google Drive</span>
          </div>
          <div className={styles.syncRow}>
            <span className={styles.syncLabel}>예산 계획</span>
            <span className={styles.syncValue}>localStorage + Google Drive</span>
          </div>
          <div className={styles.syncRow}>
            <span className={styles.syncLabel}>정기지출 목록</span>
            <span className={styles.syncValue}>localStorage (브라우저 전용)</span>
          </div>
          <div className={styles.syncRow}>
            <span className={styles.syncLabel}>설정/카테고리</span>
            <span className={styles.syncValue}>IndexedDB + Google Drive</span>
          </div>
        </div>
      </div>
      ...
      <div className={styles.section}>
        ...
        <p className={styles.sectionDesc}>
          ...
          현재 월 + 이전 3개월치 거래, 예산 계획, 정기지출, 공동지출 등이 포함됩니다.
        </p>
        <div className={styles.exportBtnRow}>
          <button
            className={styles.exportBtn}
        ...
        {seedMsg && (
          <div className={styles.exportMsg} style={seedMsg.startsWith('오류') ? { background: 'color-mix(in srgb, var(--error) 12%, var(--bg-card))', borderColor: 'var(--error)', color: 'var(--error)' } : undefined}>
            {seedMsg}
          </div>
        )}
      </div>
      ...
      {activeTab === 'general' && (
        <div className={styles.tabContent}>
          ...
          <div className={styles.section}>
            ...
            <p className={styles.sectionDesc}>이 일수 이상 기록이 없으면 홈에서 복귀 배너가 표시됩니다.</p>
          ...
          <div className={styles.section}>
            <span className={styles.sectionTitle}>동기화</span>
            <div className={styles.syncCard}>
              <div className={styles.syncRow}>
                <span className={styles.syncLabel}>마지막 동기화</span>
                <span className={styles.syncValue}>{syncTimeStr}</span>
              </div>
              <div className={styles.syncRow}>
                <span className={styles.syncLabel}>저장소</span>
                <span className={styles.syncValue}>Google Drive (appDataFolder)</span>
              </div>
            </div>
          </div>
